...
namespace QM_CapAttack
{
    ...
    [HarmonyPatch(typeof(Creature), nameof(Creature.MeleeAttack),
        typeof(int), typeof(int), typeof(Creature), typeof(BasePickupItem), typeof(bool))]
    public static class Monster_MeleeAttack_Patch
    {
        public static bool Prefix(Creature __instance)
        {
            ...
            {
#if DEBUG
                Debug.LogWarning($"Attacks Reached: {__instance.CreatureData.ActorId} {AiBehaviour_ProcessActionPoint__Patch.MeleeAttacks}");
#endif
                return false;
            }

#if DEBUG
            Debug.Log($"Allow Attack: {__instance.CreatureData.ActorId} {AiBehaviour_ProcessActionPoint__Patch.MeleeAttacks}");
#endif
            return true;
        }
    }
